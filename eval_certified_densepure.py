...
from runners.consistency_distillation_edm_cifar10 import ConsistencyDiffusion
from runners.edm_multistep import EDM
from runners.edm_onestep import EDM_onestep
from runners.edm_multistep_ours import EDM_ours_multistep
...
class DensePure_Certify(nn.Module):
    def __init__(self, args, config):
        ...
        if args.domain == 'cifar10':
            if args.advanced_classifier=='vit':
                ...
                self.classifier = AutoModelForImageClassification.from_pretrained("aaraki/vit-base-patch16-224-in21k-finetuned-cifar10").to(config.device)
                ...
                self.classifier = checkpoint['net'].to(config.device)
            ...
            if args.advanced_classifier=='beit':
                self.classifier = timm.create_model('beit_large_patch16_512', checkpoint_path='pretrained/beit_large_patch16_512_pt22k_ft22kto1k.pth').to(config.device)
                self.classifier.eval()
            elif args.advanced_classifier=='WRN':
                self.classifier = timm.create_model('wide_resnet50_2', pretrained=True).to(config.device)
                self.classifier.eval()
            elif args.advanced_classifier=='MLP':
                self.classifier = timm.create_model('mixer_b16_224_miil', pretrained=True).to(config.device)
                self.classifier.eval()
            elif args.advanced_classifier=='resnet':
                self.classifier = timm.create_model('resnet152', pretrained=True).to(config.device)
                self.classifier.eval()
            else:
                raise NotImplementedError('no classifier')
        else:
        ...
        if args.diffusion_type == 'guided-ddpm':
            ...
            self.runner = ConsistencyDiffusion(device=config.device)
        elif args.diffusion_type == 'edm':
            self.runner = EDM(sigma=self.args.sigma,device=config.device)
        elif args.diffusion_type == 'edm_onestep':
            self.runner = EDM_onestep(sigma=self.args.sigma,device=config.device)
        elif args.diffusion_type == 'edm_ours_multistep':
            self.runner = EDM_ours_multistep(device=config.device)
        else:
    ...
    def forward(self, x, sample_id):
        ...
        if self.args.advanced_classifier=='vit':
            self.classifier.eval()
            x_re = (x_re+1)*0.5*255
            x_re = F.interpolate(x_re.int().float(), size=(224, 224), mode='bilinear')
            x_re = x_re/255*2-1
            ...
            if self.args.vit_batch!=0:
                ...
                for i in range(batch_num):
                    if i==(batch_num-1):
                        ...
                        out[i*self.args.vit_batch:] = self.classifier(x_re[i*self.args.vit_batch:]).logits
                    else:
                        ...
                        out[i*self.args.vit_batch:(i+1)*self.args.vit_batch] = self.classifier(x_re[i*self.args.vit_batch:(i+1)*self.args.vit_batch]).logits
            else:
                # inputs = self.extractor(x_re, return_tensors="pt")
                # out = self.classifier(inputs["pixel_values"].cuda()).logits
                out = self.classifier(x_re).logits

        elif self.args.advanced_classifier=='cifar-wrn':
            self.classifier.eval()
            x_re = (x_re+1)*0.5
            means = torch.tensor([0.4914, 0.4822, 0.4465]).to(config.device)
            sds = torch.tensor([0.2471, 0.2435, 0.2616]).to(config.device)
        ...
        if self.args.advanced_classifier=='vit':
            ...
            if self.args.vit_batch!=0:
                ...
                for i in range(batch_num):
                    if i==(batch_num-1):
                        inputs = self.extractor(x_re[i*self.args.vit_batch:], return_tensors="pt")
                        out[i*self.args.vit_batch:] = self.classifier(inputs["pixel_values"].to(config.device)).logits
                    else:
                        inputs = self.extractor(x_re[i*self.args.vit_batch:(i+1)*self.args.vit_batch], return_tensors="pt")
                        out[i*self.args.vit_batch:(i+1)*self.args.vit_batch] = self.classifier(inputs["pixel_values"].to(config.device)).logits
            else:
                inputs = self.extractor(x_re, return_tensors="pt")
                out = self.classifier(inputs["pixel_values"].to(config.device)).logits
...
def original_certify(dataset, args, config):
    ...
    smoothed_classifier = Smooth(classifier, get_num_classes(args.domain), args.sigma,device=args.device)
    ...
    if args.use_id:
        for i in args.sample_id:
            ...
            label = torch.tensor(label,dtype=torch.int).to(config.device)
...
def purified_certify(model, dataset, args, config):
    ...
    smoothed_classifier_diffpure = Smooth(model, get_num_classes(args.domain), args.sigma, device=args.device)
...
def parse_args_and_config():
    ...
    parser.add_argument('--diffusion_type', type=str, default='ddpm', help='[ddpm, sde, cm]')
    ...
    parser.add_argument("--device", type=str, default='0', help="gpu/cpu specification")
